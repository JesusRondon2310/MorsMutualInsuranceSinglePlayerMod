...
namespace MMI_SP.Agency.Office.Ambient
{
    internal static class CollectionPicker
    {
        ...
        public static Result<OfficeItemsCollection> Pick()
        {
            ...
            if (hour >= Constants.MORNING_START_HOUR && hour < Constants.NOON_START_HOUR)
                pool.AddRange(ItemsManager.GetItemsCollection(CollectionType.Normal));
            else if (hour >= Constants.NOON_START_HOUR && hour < Constants.AFTERNOON_START_HOUR)
            {
                pool.AddRange(ItemsManager.GetItemsCollection(CollectionType.Midday));
                pool.AddRange(ItemsManager.GetItemsCollection(CollectionType.Normal));
            }
            else if (hour >= Constants.AFTERNOON_START_HOUR)
                pool.AddRange(ItemsManager.GetItemsCollection(CollectionType.Normal));
            else
            {
                pool.AddRange(ItemsManager.GetItemsCollection(CollectionType.Normal));
            ...
            return new Ok<OfficeItemsCollection>(pool[rnd.Next(Constants.FIRST_INDEX, pool.Count)]);
        }
    }
}