...
namespace MMI_SP.Agency
{
    static class ItemsManager
    {
        ...
        internal class OfficeItemsCollection : List<OfficeItem>
        {
            ...
            {
                for (int i = Count - Constants.ONE; i >= Constants.ZERO; i--)
                    DeleteItemAt(i);
            }

            public Result<bool> DeleteItemAt(int i)
            {
                if (i < Constants.ZERO || i >= Count) return new Err<bool>("Índice fuera de rango.");
        ...
        internal class OfficeItem
        {
            ...
            {
                for (int i = Constants.FIRST_INDEX; i < Constants.MAX_PROP_CREATION_ATTEMPTS; i++)
                {
