...
namespace MMI_SP.Helpers.Spawn
{
    public static class VehicleSpawnManager
    {
        ...
        // BLOQUE 2: Funciones
        // ==========================================
        public static Result<Vehicle> SpawnVehicle(VehicleData data, bool validatePosition = true)
        {
            ...
            if (validatePosition) {
                ...
                if (nodeResult.is_ok())
                    spawnPos = ((Ok<Vector3>)nodeResult).Value;
            ...
            // Aplicar liveries (mod 22) de forma específica si existe
            ...
            else if (data.Mods.ContainsKey(23)) Function.Call(Hash.SET_VEHICLE_MOD, veh, 23, data.Mods[23], false);
            ...
            // Ajuste final: forzar el heading original del JSON
