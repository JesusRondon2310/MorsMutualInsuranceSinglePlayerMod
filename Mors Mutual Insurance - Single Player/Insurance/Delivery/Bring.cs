...
namespace MMI_SP.Insurance.Delivery
{
   internal static class Bring
   {
          ...
          Dictionary<string, Blip> blipsToRemove, List<Data> incomingVehicles)
      {
         ...
         // Caso 3: distancia entre 200m y 600m o isLocked: true -> TELEPORT
         if (isLocked || (distance >= Constants.MIN_DISTANCE_FOR_DELIVERY && distance < Constants.DORMANCY_THRESHOLD && !isInGarage && 
         !isInImpound)) {
