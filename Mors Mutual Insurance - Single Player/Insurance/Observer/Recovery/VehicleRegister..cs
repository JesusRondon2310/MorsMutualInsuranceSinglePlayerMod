...
{
    internal static class VehicleRegister
    {
        internal static void RegisterRecoveredVehicle(Vehicle veh, Blip blip, List<Vehicle> recoveredVehList, Dictionary<string, Blip> blipsToRemove)
        {
            if (!recoveredVehList.Contains(veh)) recoveredVehList.Add(veh);

            string key = VehicleIdentifier.Get(veh);
            if (!blipsToRemove.ContainsValue(blip) && !blipsToRemove.ContainsKey(key)) blipsToRemove.Add(key, blip);
        ...
        {
            for (int i = recoveredVehList.Count - Constants.ONE; i >= Constants.ZERO; i--)
            {
