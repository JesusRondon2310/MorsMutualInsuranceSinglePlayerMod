...
namespace MMI_SP.Insurance.Observer
{
   internal static class Initializer
   {
      ...
      {
         var spawnResult = VehicleSpawnManager.SpawnVehicle(data, adjustToRoadNode: false);
      ...
      internal static void RestoreVehiclesFromDatabase(List<Vehicle> insuredVehList, Dictionary<string, Blip> blipsToRemove)
      {
         ...
         foreach (VehicleData data in allVehicles)
         {
            ...
            if (data.IsDormant) {
               Dormancy.Core.MarkAsDormant(data.Id); 
               continue;
            }
            ...
            if (existing != null) {
               // Actualizar la BD con la posición actual del vehículo existente
               var updatedData = data.With(d =>
               {
                  d.PosX = existing.Position.X;
                  d.PosY = existing.Position.Y;
                  d.PosZ = existing.Position.Z;
                  d.Heading = existing.Heading;
               });
            ...
            if (distanceToPlayer >= Constants.DORMANCY_THRESHOLD) {
               if (data.IsLocked) TryAddVehicleFromData(data, insuredVehList, blipsToRemove);
               
               else Dormancy.Core.MarkAsDormant(data.Id);
               continue;
            }
