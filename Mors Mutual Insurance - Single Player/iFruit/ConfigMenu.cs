...
namespace MMI_SP.iFruit
{
    internal class ConfigMenu
    {
        ...
        {
            _pool = new MenuPool();
            _mainMenu = new UIMenu("Configuración", "Ajustes del mod");
            _pool.Add(_mainMenu);
        ...
        private void BuildiFruit()
        {
            ...
            ConfigMenuBuilder.AddListInt(sub, "iFruit", "PhoneVolume", ModSettings.iFruitVolume, "Volumen del teléfono", 0, 100, 5);

            ConfigMenuBuilder.AddActionItem(sub, "Buscar vehículos perdidos", () =>
            {
                Helpers.VehicleRecoveryHelper.RecoverLostVehicles();
            });

            ConfigMenuBuilder.AddActionItem(sub, "Eliminar duplicados del lote", () =>
            {
                Helpers.VehicleRecoveryHelper.CleanDuplicatesInImpound();
            });
        }
