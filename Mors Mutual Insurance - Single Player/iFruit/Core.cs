...
using iFruitAddon2;
using MMI_SP.Debug;
using MMI_SP.Helpers; // Para usar Timers
...
namespace MMI_SP.iFruit
{
    public class Core : Script
    {
        ...
        // Eliminado: private int _closePhoneTime;
        ...
        private void Initialize(object sender, EventArgs e)
        {
            ...
            _menu = new MMIMenu(); _configMenu = new ConfigMenu(); _mechanicMenu = new MechanicMenu();
            ...
            _iFruit.Contacts.Add(contactMMI); _iFruit.Contacts.Add(contactMechanic); _iFruit.Contacts.Add(contactConfig);

            Tick -= Initialize; Tick += OnTick;
        }

        private void OnTick(object sender, EventArgs e)
        {
            _menu?.MenuPoolProcessMenus(); _configMenu?.MenuPoolProcessMenus(); _mechanicMenu?.MenuPoolProcessMenus();

            Manager.ForceArrivalIfNear();

            // Ejecutar callback del timer de cierre del teléfono si expiró
            ...
            _wasMenuVisible = isMMIVisible; _wasMechanicMenuVisible = isMechanicVisible;

            if (!Function.Call<bool>(Hash.IS_MOBILE_PHONE_CALL_ONGOING)) _iFruit.Update();
        }

        private void OnAborted(object sender, EventArgs e) { if (_iFruit?.Contacts?.Count > 0) _iFruit.Contacts.ForEach(c => c.EndCall()); }

        private void OnMMIAnswered(iFruitContact contact) => ExecuteSafe("OnMMIAnswered", () =>
        {
            MMISound.Play(MMISound.SoundFamily.Hello);
            _menu.Reset(true); _menu.Show();
            _wasMenuVisible = true;
            Timers.StartCustomTimer("ClosePhone", 2000, () => _iFruit.Close());
        });

        private void OnConfigAnswered(iFruitContact contact) => ExecuteSafe("OnConfigAnswered", () =>
        {
            _configMenu.Show();
            _iFruit.Close();
        });

        private void OnMechanicAnswered(iFruitContact contact) => ExecuteSafe("OnMechanicAnswered", () =>
        {
            MechanicSound.Play(MechanicSound.SoundFamily.Hello);
            _mechanicMenu.Reset(true); _mechanicMenu.Show();
